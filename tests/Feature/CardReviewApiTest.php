...
test('recording a review updates memory data and creates immutable history', function () {
    ...
    $learningRecord = LearningRecord::factory()->create([
        'user_id' => $user->id,
        'card_id' => $card->id,
        'due_at' => now(),
    ...
    $rating = Rating::query()
        ->whereBelongsTo($card->type, 'cardType')
        ->where('recall_quality', RecallQuality::Remembered)
        ->firstOrFail();
    $reviewedAt = now()->startOfSecond();
    $this->travelTo($reviewedAt);

    $this->actingAs($user)->postJson("/api/v1/cards/{$card->id}/reviews", [
        'rating_id' => $rating->id,
        'duration_ms' => 4200,
    ])
        ->assertCreated()
    ...
    $this->assertDatabaseHas('card_reviews', [
        ...
        'rating_id' => $rating->id,
        'scheduler_version' => 'fsrs_6',
        'interval_before_minutes' => 0,
        'interval_after_minutes' => 3321,
        'duration_ms' => 4200,
    ]);
});
...
test('a rating belonging to another card type is rejected', function () {
    ...
    $rating = Rating::query()
        ->whereHas('cardType', fn ($query) => $query->where('name', CardTypeName::Note))
        ->firstOrFail();
...
test('a user cannot review another users card', function () {
    ...
    $this->actingAs($otherUser)
        ->postJson("/api/v1/cards/{$card->id}/reviews", ['rating_id' => $rating->id])
        ->assertForbidden();
