...
test('successful recall schedules a future review', function () {
    $learningRecord = new LearningRecord(['review_count' => 0, 'relearning_count' => 0, 'interval_minutes' => 0, 'learning_state' => LearningState::New]);
    ...
    expect($schedule['learning_state'])->toBe(LearningState::Reviewing)
        ->and($schedule['interval_minutes'])->toBe(10080)
...
test('a missed learned card enters relearning and counts a lapse', function () {
    $learningRecord = new LearningRecord(['review_count' => 4, 'relearning_count' => 1, 'interval_minutes' => 17280, 'learning_state' => LearningState::Reviewing]);
    $reviewedAt = CarbonImmutable::parse('2026-09-13 10:00:00');
    ...
    expect($schedule['learning_state'])->toBe(LearningState::Relearning)
        ->and($schedule['interval_minutes'])->toBe(10)
