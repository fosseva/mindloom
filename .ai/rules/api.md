...
## Load relationships only through requested includes
Do not eager load relationships directly in API controllers with `with()`, `load()`, or `loadMissing()`. Declare relationships with Spatie Query Builder `allowedIncludes()` and load them only when the client explicitly requests the corresponding `include=` value.
